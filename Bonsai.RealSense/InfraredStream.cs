...
{
    [Description("Subscribes to the native stream of infrared images in a RealSense device.")]
    public class InfraredStream : Combinator<Device, VideoDataFrame>
    {
        ...
        public override IObservable<VideoDataFrame> Process(IObservable<Device> source)
        {
            return source.SelectMany(device =>
            {
                ...
                var profile = depthSensor.StreamProfiles.First(p => p.Framerate == Framerate && p.Stream == Stream.Infrared);
                return Observable.Create<VideoDataFrame>(observer =>
                {
                    ...
                    depthSensor.Start(frame =>
                    {
                        ...
                        {
                            var image = new IplImage(new Size(videoFrame.Width, videoFrame.Height), IplDepth.U8, 1);
